...
# --- Basic parameters ---
nx, ny = vnt_map.data.shape[1], vnt_map.data.shape[0]

# X-axis: raster time (force 0 → 10000 s)
t_array = np.linspace(0, 10000, nx)

# Y-axis: solar slit positions (arcsec)
slit_pos = vnt_map.meta['crval2'] + vnt_map.meta['cdelt2'] * (np.arange(ny) - vnt_map.meta['crpix2'])
...
dopp_line = np.nan_to_num(vnt_map.data[y_index, :], nan=np.nanmean(vnt_map.data))
